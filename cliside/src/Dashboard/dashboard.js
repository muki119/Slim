...
function Dashboard (props){
    ...
    const [success_cc,sets_cc]=useState(false)
    ...
    if (dashdata){
        if (dashdata.redirect === false){ // if not authorized 
            ...
            var convomp = chats.data.map((element,index)=>{ // maps the lists into tiles clickable tiles 
                ...
                return <div key = {index}id={element.chat_id} onClick={chatchanger} tabIndex={0}><p onClick={(e)=>{e.preventDefault() }}class= 'chatname'>{usersinvolved.map((users)=>{return users+' '})}</p></div> // id for the chat_id used -chatchanger is a function that changes the conversation by making the new one a 
            })

            return(
                <div class = 'dbackground' >
                    <div class='dinbox'>
                        {displaycc === true && <CreateChatContext.Provider value={{chats,setchats,displaycc,setcc,forceUpdate,socket,sets_cc}}><CreateChat/></CreateChatContext.Provider>}
                        <div class ='topbar'>
